module block_13 (
  ...
  assign n0043 = u0089; //bit0 in counter
  assign n0042 = u0118; //bit3 in counter
  assign n0045 = u0129; //bit1 in counter
  assign n0046 = u0141; //bit2 in counter
  assign u0101 = n0248;
  ...
  //n0446 / n0447 generated from block_6
  // any 1 in input -> true
  // the 4 inputs are the low 4 bits of block 6
  // when counter not 0 -> high

  sky130_fd_sc_hd__or4bb u0257 (.X(n0171), .C_N(n0045), .D_N(n0046), .A(n0043), .B(n0042));  // x=83.260 y=104.720
  //n0171 generated from block_6
  //42 43 high or 45 46 low -> true
  //4bit counter high low bit high or mid bit low
  //counter not 6 -> high

  sky130_fd_sc_hd__a22o u0247 (.B2(n0312), .B1(n0446), .A1(n0447), .A2(n0209), .X(n0169));  // x=85.100 y=161.840
  //n0169 generated from shift register[12,1] & n0446
  //only interested in 2 bits?
  //when 1,12 both true, and any 1 in input -> true
  //when 1,12 both true, and counter not 0 -> true

  sky130_fd_sc_hd__a221o u0066 (.C1(n0169), .B2(n0170), .B1(1'b1), .A1(n0171), .A2(n0172), .X(n0173));  // x=70.610 y=150.960
  //n0173 generated from shift register[10,11] & n0169 & n0171
  //interesting in other 2 bits?
  //when 169 is true, or 11 true, or 171 and 10 -> true
  //when 169 is true, or 11 true, or counter not 6 and 10 -> true
  
  sky130_fd_sc_hd__a31o u0398 (.X(n0249), .A3(n0173), .A2(n0025), .A1(I), .B1(n0248));  // x=72.450 y=156.400
  //n0249 generated from shift register & n0173 & u0101 & input
  //when input high & 173 & enable or u0101 high -> true
  //self-lock, when high keep high

  sky130_fd_sc_hd__dfrtp ff_u0101 (.RESET_B(rst_n), .Q(n0248), .CLK(clk), .D(n0249));  // x=92.690 y=150.960
  //final output 
  //when input is true
  //  if counter == 0, 10/11 before is 1
  //  if counter == 6, 1/11/12 before is 1
  //  otherwise, 1/10/11/12 before is 1
  // set n0248
